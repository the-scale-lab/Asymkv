...
from  asymkv.util.cache_utils import apply_rotary_pos_emb_single, CompressCache
import types
import logging
...
def enable_mistral_asymkv_attention_recursive(model):
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            enable_mistral_asymkv_attention_recursive(
                module,
            )

        if isinstance(module, MistralAttention):
            model._modules[name].forward = types.MethodType(
                MistralAttention_forward, model._modules[name]
            )

def enable_mistral_asymkv_attention(model):
    model.model.forward = types.MethodType(MistralModel_forward, model.model)
    enable_mistral_asymkv_attention_recursive(model)
